<?php
// sidebarVet.php - panel lateral para Recepcionista con el mismo estilo visual que el panel admin
if (session_status() !== PHP_SESSION_ACTIVE) session_start();
$usuario = $_SESSION['usuario'] ?? '';
$rol = $_SESSION['rol'] ?? 'Recepcionista';
...
/* Sidebar base (coherente con admin) */
.sidebarRecep{
  position: fixed;
  top: 0;
  left: 0;
...
/* Nav */
.sidebarRecep nav{ display:flex; flex-direction:column; gap:6px; padding:4px; }
.sidebarRecep a{
  display:flex;
  align-items:center;
  gap:12px;
...
.sidebarRecep a:hover{
  background: var(--hover-bg);
  transform: translateY(-2px);
  box-shadow: 0 6px 14px rgba(0,0,0,0.06);
}
.sidebarRecep a.active{
...
.sidebarRecep a span.label{ flex:1; text-align:left; }
...
.sidebarRecep.compact{ width: var(--sidebar-compact); }
.sidebarRecep.compact a{ justify-content:center; padding:10px 6px; }
.sidebarRecep.compact a span.label{ display:none; }
.sidebarRecep.compact .brand{ display:none; }
.sidebarRecep.compact .footer{ display:none; }

/* Tooltip in compact mode */
.sidebarRecep.compact a[title]:hover::after{
  content: attr(title);
  position: absolute;
  left: calc(100% + 8px);
  top: 50%;
...
@media (max-width:900px){
  .sidebarRecep{ position: relative; width:100%; height:auto; flex-direction:row; flex-wrap:wrap; padding:8px; gap:6px; }
  .sidebarRecep .brand{ flex-basis:100%; text-align:left; padding-left:12px; }
  .main-wrapper{ margin-left:0; } /* páginas deben manejar el margen principal */
  .sidebarRecep.compact{ width:100%; }
}

/* Accessibility focus */
.sidebarRecep a:focus{
  outline: 3px solid rgba(0,0,0,0.12);
  outline-offset: 2px;
}
</style>

<div class="sidebarRecep" role="navigation" aria-label="Panel lateral recepcionista">
  <div class="brand" aria-hidden="true">RECEPCIONISTA
  ...
  <nav>
    <a href="recep_panel.php" class="<?= $current === 'recep_panel.php' ? 'active' : '' ?>" title="Panel">
    ...
    <a href="crud_cita.php" class="<?= $current === 'crud_cita.php' ? 'active' : '' ?>" title="Agendar Cita">
      <span class="icon" aria-hidden="true">
    ...
    <a href="crud_cliente.php" class="<?= $current === 'crud_cliente.php' ? 'active' : '' ?>" title="Clientes">
      <span class="icon" aria-hidden="true">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 12c2.21 0 4-1.79 4-4S14.21 4 12 4 8 5.79 8 8s1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
      </span>
      <span class="label">Clientes</span>
    </a>
    <a href="mascotas.php" class="<?= $current === 'crud_pago.php' ? 'active' : '' ?>" title="Pago">
      <span class="icon" aria-hidden="true">
    ...
    <a href="crud_pago.php" class="<?= $current === 'crud_pago.php' ? 'active' : '' ?>" title="Pago">
      <span class="icon" aria-hidden="true">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M21 7H3V5h18v2zM3 19h18v-8H3v8zm2-6h14v4H5v-4z"/></svg>
      </span>
  ...
  <div class="footer" aria-hidden="false">
    <div style="font-weight:700; color:#fff;">Rol: <span style="font-weight:800; margin-left:6px;"><?php echo htmlspecialchars($rol); ?></span></div>
    <?php if ($usuario): ?>
...
(function(){
  function updateCompact(){
    const sb = document.querySelector('.sidebarRecep');
    if (!sb) return;
